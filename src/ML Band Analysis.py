...
"""
Takes one of the .data files as an input and performs machine learning on each of the features of each individual band, 
using a DNN. It then writes the results of 10-cross validation to a .txt file
...
if __name__ == "__main__":
    ...
    res = []
    # Iterates over the frequency bands
    for i in range(4):
        res2 = []
        for j in range(10):
            # Declares the index ranges for the band features
            upper = int(round(float(len(data[0])) * float(i+1) / 4.0))
            lower = int(round(float(len(data[0])) * float(i) / 4.0))
    ...
    # Writes each band's results to the output file
    bands = ['Theta', 'Alpha', 'Beta', 'Gamma']
    with open(output_path + "/values.txt", "w", newline='') as f_res:
        for i in range(len(res)):
            f_res.write(bands[i] + ": " + str(100 * np.array(res[i])[0]) + " +- " + str(100 * np.array(res[i])[1]) + "\n")
