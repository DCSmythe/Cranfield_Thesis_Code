...
"""
Takes one of the .data files as an input and performs machine learning on the 2 class data within that file. It then
...
if __name__ == "__main__":
    ...
    for i in range(itrs):
        ...
        for j in range(len(Y3)):
            results_array[int(Y2[j][1])][int(Y3[j][1])] += 1
    ...
    with open(output_path + "/values.txt", "w", newline='') as f_res:
        f_res.write(
            "Accuracy:  " + str(np.mean(np.array(res), axis=0)[1]) + " +- " + str(np.std(np.array(res), axis=0)[1]) + "\n")
        f_res.write(
            "Precision: " + str(np.mean(np.array(res), axis=0)[2]) + " +- " + str(np.std(np.array(res), axis=0)[2]) + "\n")
        f_res.write(
            "Recall:    " + str(np.mean(np.array(res), axis=0)[3]) + " +- " + str(np.std(np.array(res), axis=0)[3]) + "\n")
