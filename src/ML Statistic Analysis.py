...
"""
Takes one of the .data files as an input and performs machine learning on each of the features corresponding to each
individual statistic, using a DNN. It then writes the results of 10-cross validation to a .txt file
...
if __name__ == "__main__":
    ...
    res = []
    # Iterates over each statistic
    for i in range(4):
        res2 = []
        for j in range(10):
            # Declares the index ranges for each statistic's features
            indices = np.array(range(i, len(data[0]), 4))
            ...
            data_train = np.append(data[0:lower2, indices], data[upper2:, indices], axis=0)
            labels_train = np.append(labels[0:lower2, :], labels[upper2:, :], axis=0)
            data_test = np.array(data[lower2:upper2, indices])
    ...
    # Writes each statistic's results to the output file
    stat = ['Power Ratio', 'Mean', 'Variance', 'Abs Max']
    with open(output_path + "/values.txt", "w", newline='') as f_res:
        for i in range(len(res)):
            f_res.write(stat[i] + ": " + str(100 * np.array(res[i])[0]) + " +- " + str(100 * np.array(res[i])[1]) + "\n")